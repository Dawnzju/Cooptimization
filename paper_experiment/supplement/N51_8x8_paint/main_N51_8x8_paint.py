...
import time
from handle_output import handle_my_output,MyEncoder
import os
...
#init input
hyperperiod,num_of_tasks,edges,comp_cost=init('./task graph/N51_H264.tgff')
...
computation_ability=read_NoC('./NoC description/N51_H264_Mesh8x8_NoCdescription.txt')
...
for i in range(0,150):
    ...
    flag,cur_task_graph=Iteration(num_of_tasks,1,N)
    ...
    if((i+1) % 10 ==0):
        ...
        print("---------------",file=f)
    exe_sum=0
    for j in Tasks_position_current_solution.keys():
        position=Tasks_position_current_solution[j]
        exe_sum+=computation_ability[j][position]
    print(exe_sum,flag,file=f2)
    cur_json=handle_my_output('./task graph/N51_H264.tgff',Tasks_position_current_solution,cur_task_graph,computation_ability)
    dir_name='./paint_json/'
    if(os.path.exists(dir_name)==False):
        os.makedirs(dir_name)
    with open(dir_name+'H264_8x8_AIR1_iter_'+str(i)+'.json',"w") as f3:
        f3.write(json.dumps(cur_json,cls=MyEncoder))
