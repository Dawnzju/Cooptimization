...
weight_a,weight_b=Get_weight(total_needSend,execution,c=5)

#computation_ability=CVB_method(execution=execution[1:],V_machine=0.25,num_of_rows=N)
computation_ability=read_NoC('./NoC description/N22_audiobeam_Mesh8x8_NoCdescription_V0.25.txt')

for i in range(0,num_of_tasks):
    for j in range(0,len(computation_ability[i])):
        if(computation_ability[i][j]==0):
            computation_ability[i][j]=1
...
print("]",file=f)
f.close()
ret_json=handle_my_output('./task graph/N22_audiobeam.tgff',Tasks_position_best_solution,best_task_graph,computation_ability)
with open('audiobeam_8x8_AIR1_V0.25_c=5_copoti_diff.json',"w") as f3:
    f3.write(json.dumps(ret_json,cls=MyEncoder))
print("done")
